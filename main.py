...
def clean_concatenate(fields):
    ...
    field_values = [str(field) for field in fields if field]  # Convert to string and remove empty values
...
for index, row in df.iterrows():
    ...
    address_fields = clean_concatenate([row[field] for field in ['ADDRESS', 'pincodes', 'CircleName', 'RegionName', 'DivisionName', 'OfficeName', 'StateName']])

    # Concatenate, clean, and limit contact fields
    contact_fields = clean_concatenate([row[field] for field in ['Mobile/Contact Number', 'Contact number', 'Other Contacts']])
    contact_fields = limit_numbers(contact_fields)

    # Replace placeholders in the template with actual values
    output = template.render(schoolname=row['SCHOOL'], address=address_fields, contact=contact_fields)
